...
use crate::{
    ExportFormat, exit_confirmation::SiteChanged, recency::RecencyRanking, site::*,
...
/// Generate a ROS 2 launch file (Python) for the exported SDF world.
#[cfg(not(target_arch = "wasm32"))]
fn generate_ros2_launch_file(world_file: &str) -> String {
    format!(
        r#"""ROS 2 launch file for Gazebo simulation.

Generated by RMF Site Editor.
Usage:
  ros2 launch {launch_py} [world_path:=<path>]

Or directly with Gazebo:
  gz sim {world_file}
"""
...
def generate_launch_description():
    ...
    default_world = os.path.join(launch_dir, '{world_file}')
